...
import { place, type Config } from './commands'

describe('place', () => {
  it('should return the specified config if inside the bounds', () => {
    const validConfig: Config = { x: 1, y: 2, f: 'NORTH' }
    expect(place(validConfig)).toEqual(validConfig)
  })

  it('should throw an error when the config is out of bounds in the x direction', () => {
    const invalidConfig: Config = { x: 5, y: 2, f: 'NORTH' }
    expect(() => place(invalidConfig)).toThrow()
  })
